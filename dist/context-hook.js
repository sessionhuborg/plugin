...
function formatTableRow(obs, index) {
...
function formatFullObservation(obs, index) {
  const emoji = TYPE_EMOJI_MAP[obs.type];
  const date = new Date(obs.created_at).toLocaleDateString("en-US", {
    month: "short",
    day: "numeric",
...
function formatObservationsContext(observations, options = {}) {
  const {
    topN = 10,
    includeTable = true,
...
// src/context-hook.ts
var sessionhubDir = import_path2.default.join((0, import_os.homedir)(), ".sessionhub");
try {
...
async function main() {
  try {
    ...
    const projectName = import_path2.default.basename(projectPath);
    ...
    const observations = obsResult.observations.map((obs) => ({
      id: obs.id,
      ...
      discovery_tokens: 0,
      // Not tracked in backend response
